...
public class CustomerServiceImpl {
	...
	public List<CustomerVO> findAllCustomers(){
		...
		for(CustomerEntity entity :entities) {
			...
			if(entity.getAddressEntity()!=null){
				...
				customerVO.setStreet(entity.getAddressEntity().getStreet());	
			}
	...
	public String save(CustomerVO customerVO){
		...
		customerEntity.setPassword(customerVO.getPassword()); //Encrypting password using spring password encoder
		...
		addressEntity.setStreet(customerVO.getStreet());
		addressEntity.setDoe(new Timestamp(new Date().getTime()));
		addressEntity.setZipCode(12232);
...
//	public CustomerVO findByDebitCard(String debitcard) {
//		CustomerVO customerVO=null;
//		Optional<CustomerEntity> optional=customerDaoRepository.findByDebitCard(debitcard);
//		if(optional.isPresent()){
//			CustomerEntity customerEntity=optional.get();
//			customerVO=new CustomerVO();
//			BeanUtils.copyProperties(customerEntity, customerVO);
...
//	}
	...
	public String updateRolesForCustomer(UpdateRoles updateRoles) {
		...
		for(Integer crid:updateRoles.getSroles()){
			...
			RoleEntity entity=roleDaoRepository.findById(crid).get();
			ndroles.add(entity);
		}
	...
	public String updateCustomer(CustomerVO customerVO) {
		CustomerEntity customerEntity=customerDaoRepository.findById(customerVO.getCid()).get();
	...
	@Transactional
	public String changePassword(CustomerVO customerVO) {
		customerVO.setPassword(encoder.encode(customerVO.getPassword()));
		CustomerEntity customerEntity=customerDaoRepository.findById(customerVO.getCid()).get();
